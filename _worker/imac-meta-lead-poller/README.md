...
| 인프라 | 워커 `/api/health` · D1 읽기 1행 · R2 자산 HEAD · **미등록 활성폼**(6시간마다) | 워커·D1 · 미등록폼에 리드가 있을 때 |
| 아이맥 | 디스크·메모리·절전설정·launchd 폴러 상태 | 디스크만 |

### 미등록 활성폼 감지가 왜 필요한가

조회 대상은 `META_LEAD_FORM_IDS` **정적 목록**이다. 새 리드폼을 만들고 여기 추가를 잊으면
그 폼 리드는 전량 유실되는데, **폴러는 멀쩡히 살아있으니 리포트엔 "신규 없음"만 찍힌다.**
장애 알림 하나 없이 접수가 새는 게 제일 위험하다 — 정지는 최소한 실패 알림이라도 뜬다.

그래서 페이지의 ACTIVE 폼 목록과 대조해 빠진 게 있으면 알린다.
이미 리드가 쌓인 폼이 빠져 있으면 유실이 확정된 것이라 🔴, 리드 0건이면(방금 만든 폼일 수 있다) ⚠️.
**발견해도 조회 대상에 자동 추가하지 않는다** — 모르는 폼을 말없이 접수하는 게 더 위험하다.
사람이 `META_LEAD_FORM_IDS` 에 넣게 한다.

Graph 왕복 2번(~1.9초)이라 매시간은 낭비다. 6시간마다만 돌고 사이 시간대엔 직전 결과를 재사용한다
(안 그러면 판정이 깜빡여 복구 오보가 난다). `META_LEAD_FORM_CHECK_HOURS` 로 조정.

### 전화 필드 인식은 워커와 반드시 같아야 한다

폴러 `hasPhone()` 이 워커 `META_FIELD_RULES` 의 phone 규칙보다 **좁으면 리드가 영구 유실된다** —
워커는 읽을 수 있는 리드를 폴러가 먼저 스킵하고, 스킵은 `processed` 에 처리완료로 남아
다시 안 가져온다(조회창 48시간이라 나중에 고쳐도 재수집 불가).
넓으면 워커가 400 을 뱉어 매시간 실패 알림이 쌓인다.

`worker.test.mjs` 의 "폴러/워커 전화 인식 범위 동일" 테스트가 양쪽에 같은 필드명 17종을 먹여
판정이 어긋나면 실패시킨다. 한쪽만 고치면 테스트가 막는다.
