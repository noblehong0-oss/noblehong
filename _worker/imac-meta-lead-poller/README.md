...
1. **폴러 → 인프라봇**: 신규/중복/스킵이 있었거나 마지막 보고 후 6시간이 지나면 한 줄 보고.
   실패(토큰 만료·폼 삭제·네트워크)는 즉시 🔴 알림. 매시간 도배하지 않는다.
   매 실행마다 받고 싶으면 `META_LEAD_HEALTH_ALWAYS=1`.
2. **워커 크론 → 인프라봇**: 폴러가 하트비트를 3시간 넘게 안 찍으면 접수 장애로 판정.
   아이맥이 꺼지면 폴러는 스스로 죽음을 알릴 수 없으므로 이 backstop이 필요하다.
   (워커 헬스체크 크론은 6시간 주기 → 아이맥 사망 최대 감지 지연 약 9시간)
